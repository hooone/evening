...
var (
	...
	buildId               string   = "0"
	serverBinary          string   = "evening-server"
...
func main() {
	...
	//解析编译参数；*p,name,value,description
	...
	flag.BoolVar(&enterprise, "enterprise", enterprise, "Build enterprise version of evening")
	...
	flag.BoolVar(&printGenVersion, "gen-version", printGenVersion, "generate evening version and output (default: false)")
	flag.Parse()

	buildId = shortenBuildId(buildIdRaw)
	//解析package.json中的版本信息
	...
	for _, cmd := range flag.Args() {
		...
		case "build":
			clean()
			build("evening-server", "./pkg/cmd/evening-server", []string{})
...
func makeLatestDistCopies() {
	...
	latestMapping := map[string]string{
		"_amd64.deb":               "dist/evening_latest_amd64.deb",
		".x86_64.rpm":              "dist/evening-latest-1.x86_64.rpm",
		".linux-amd64.tar.gz":      "dist/evening-latest.linux-x64.tar.gz",
		".linux-amd64-musl.tar.gz": "dist/evening-latest.linux-x64-musl.tar.gz",
		".linux-armv7.tar.gz":      "dist/evening-latest.linux-armv7.tar.gz",
		".linux-armv7-musl.tar.gz": "dist/evening-latest.linux-armv7-musl.tar.gz",
		".linux-armv6.tar.gz":      "dist/evening-latest.linux-armv6.tar.gz",
		".linux-arm64.tar.gz":      "dist/evening-latest.linux-arm64.tar.gz",
		".linux-arm64-musl.tar.gz": "dist/evening-latest.linux-arm64-musl.tar.gz",
	}
...
func createDebPackages() {
	...
	createPackage(linuxPackageOptions{
		...
		homeDir:                "/usr/share/evening",
		homeBinDir:             "/usr/share/evening/bin",
		binPath:                "/usr/sbin",
		configDir:              "/etc/evening",
		etcDefaultPath:         "/etc/default",
		etcDefaultFilePath:     "/etc/default/evening-server",
		initdScriptFilePath:    "/etc/init.d/evening-server",
		systemdServiceFilePath: "/usr/lib/systemd/system/evening-server.service",

		postinstSrc:    "packaging/deb/control/postinst",
		initdScriptSrc: "packaging/deb/init.d/evening-server",
		defaultFileSrc: "packaging/deb/default/evening-server",
		systemdFileSrc: "packaging/deb/systemd/evening-server.service",
...
func createRpmPackages() {
	...
	createPackage(linuxPackageOptions{
		...
		etcDefaultFilePath:     "/etc/sysconfig/evening-server",
		initdScriptFilePath:    "/etc/init.d/evening-server",
		systemdServiceFilePath: "/usr/lib/systemd/system/evening-server.service",

		postinstSrc:    "packaging/rpm/control/postinst",
		initdScriptSrc: "packaging/rpm/init.d/evening-server",
		defaultFileSrc: "packaging/rpm/sysconfig/evening-server",
		systemdFileSrc: "packaging/rpm/systemd/evening-server.service",
...
func createPackage(options linuxPackageOptions) {
	packageRoot, _ := ioutil.TempDir("", "evening-linux-pack")
	...
	args := []string{
		...
		"--maintainer", "contact@evening.com",
	...
	if enterprise {
		name += "-enterprise"
		args = append(args, "--replaces", "evening")
	}
...
func setup() {
	runPrint("go", "install", "-v", "./pkg/cmd/evening-server")
}
...
func clean() {
	...
	rmr("dist")
	rmr("tmp")
	rmr(filepath.Join(os.Getenv("GOPATH"), fmt.Sprintf("pkg/%s_%s/github.com/evening", goos, goarch)))
}
